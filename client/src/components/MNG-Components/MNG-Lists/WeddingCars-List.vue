<template>
  <div style="height:1000px">
    <h1 class="mx-auto mt-2" style="width: 600px">wedding cars list page content</h1><hr/>
    <form @submit.prevent="sendReq()">
      ...
      <section id="service-info">
        <div>
          <h5>Cars:</h5>
          <input type="checkbox" id="cars" v-model="visibility.cars">
          <label for="checkbox">available: {{visibility.cars}}</label><br/>

          <div v-if="visibility.cars" class="service-options" id="cars">
            <span>image for car: </span><input type="file" @change="showServiceIMG($event, 'cars')"/><br/>
            ...
            <button class="btn btn-success" @click.prevent="addItem('cars')">add</button>
            <button class="btn btn-danger" @click.prevent="undoItem('cars')">undo</button>
          </div><hr/>
    ...
    <div id="showData">
      ...
      <div class="collection">
        <h5>cars</h5>
        <div class="">{{req.services.cars}}</div>
        <img :src="img" v-for="(img, index) in showImgs.cars" :key="index"/>
      </div>
      <div class="collection">
...
async saveServicesIMGS() {
      Object.keys(this.req.services).map(async (key) => {
        ...
        let res = (await MNGServices.saveImages(fd, 'WeddingCars', key, this.Token)).data
  ...
  // CHECK INFO FIELDS IN REQ OBJ
    ...
    if (!areAllFieldsFilledIn) {
      ...
      // SEND ALL REQ TO BACK-END
        let DataRes = (await MNGServices.addMember(this.req, 'MNG-WeddingCars-Member', this.Token)).data
